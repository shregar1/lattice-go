...
- **Unit of Work Transaction Management**: Repositories and Services never call `commit()` or `rollback()`. Transaction boundaries are owned exclusively by `Orchestrator` via `UOW.ExecuteInTransaction()`.
- **Mandatory DTO Inter-Layer Data Transfer**: Raw untyped JSON is never passed across layer boundaries. All data transfer uses strongly-typed Go structs (`IBaseDTO`).
...
We urge open-source developers to contribute! Please review `docs/plans/`, pick an open blueprint, submit PRs, and help expand the Go edition of Lattice.
